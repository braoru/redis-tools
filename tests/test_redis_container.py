...
class TestContainerRedis():
    """
        Class to test the redis container.
    """

    def test_systemd_running_redis(self, settings):
        """
        Test to check if systemd is running redis.
        ...
        command_redis = (
        "busctl", "get-property", "org.freedesktop.systemd1", "/org/freedesktop/systemd1/unit/redis_2eservice",
        "org.freedesktop.systemd1.Unit", "ActiveState")
        active_status = '"active"'

        # docker exec -it busctl get-property
        check_service = docker.bake("exec", "-i", container_name, command_redis)
    ...
    def test_monit_restarts_stopped_redis(self, settings):
        """
        Test to check if monit restarts a stopped redis.
        :param settings: settings of the container, e.g. container name, service name, etc.
        :return:
        """
        ...
        max_timeout = settings['redis_timeout']
    ...
    def test_monit_restarts_killed_redis(self, settings):
        """
        Test to check if monit restarts a killed redis.
        :param settings: settings of the container, e.g. container name, service name, etc.
        :return:
        """
    ...
    def test_no_error_monit_log(self, settings):
        """
        Test to check that when running the container systemd starts redis and there is no error in the monit logs.
        :param settings: settings of the container, e.g. container name, service name, etc.
        :return:
        """
